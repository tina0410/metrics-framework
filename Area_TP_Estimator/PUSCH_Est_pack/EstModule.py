...
def Est_AT(model, N_INPUTS, QU_AT, QU_OUT, n_pipelines):
    ...
    def _as_fields(quant):
        if isinstance(quant, QuType):
            return [quant.DWT, quant.FRAC, quant.IF_SIGNED]
        return list(quant)

    # TOP estimators use one QuType for a uniform tree, while the original
    # primitive API accepts a per-layer list. Normalize both representations.
    if isinstance(QU_AT, QuType):
        QU_AT = [_as_fields(QU_AT) for _ in range(layers)]
    else:
        QU_AT = [_as_fields(quant) for quant in QU_AT]
    QU_AT.append(_as_fields(QU_OUT))
    if isinstance(n_pipelines, int):
        n_pipelines = [n_pipelines] * layers
    points = N_INPUTS
    y_pred = 0
    cnt = []
