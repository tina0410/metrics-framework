...
def ModuleComplexMul(QU_IN_1: QuType, QU_IN_2: QuType, QU_OUT: QuType, N_CLK: int, QU_MODE: QuMode.TRN | QuMode.RND, OF_MODE: OfMode.WRP | OfMode.SAT, IF_RST_N: bool, METHOD: Literal['4mul', '3mul']):
    ...
    if METHOD == '4mul':
        ...
        # Mul 1: a_re * b_re
        ModuleMul(QU_IN_1=QU_IN_1, QU_IN_2=QU_IN_2, QU_OUT=QU_OUT, N_CLK=N_CLK, QU_MODE=QU_MODE, OF_MODE=OF_MODE, IF_RST_N=IF_RST_N, PORTS={'i_data_1': 'a_re', 'i_data_2': 'b_re', 'o_data': 'prod_ar_br', **({'i_clk': 'i_clk', **({'i_rst_n': 'i_rst_n'} if IF_RST_N else {})} if N_CLK > 0 else {})})  # type: ignore

        # Mul 2: a_im * b_im
        ModuleMul(QU_IN_1=QU_IN_1, QU_IN_2=QU_IN_2, QU_OUT=QU_OUT, N_CLK=N_CLK, QU_MODE=QU_MODE, OF_MODE=OF_MODE, IF_RST_N=IF_RST_N, PORTS={'i_data_1': 'a_im', 'i_data_2': 'b_im', 'o_data': 'prod_ai_bi', **({'i_clk': 'i_clk', **({'i_rst_n': 'i_rst_n'} if IF_RST_N else {})} if N_CLK > 0 else {})})  # type: ignore

        # Mul 3: a_re * b_im
        ModuleMul(QU_IN_1=QU_IN_1, QU_IN_2=QU_IN_2, QU_OUT=QU_OUT, N_CLK=N_CLK, QU_MODE=QU_MODE, OF_MODE=OF_MODE, IF_RST_N=IF_RST_N, PORTS={'i_data_1': 'a_re', 'i_data_2': 'b_im', 'o_data': 'prod_ar_bi', **({'i_clk': 'i_clk', **({'i_rst_n': 'i_rst_n'} if IF_RST_N else {})} if N_CLK > 0 else {})})  # type: ignore

        # Mul 4: a_im * b_re
        ModuleMul(QU_IN_1=QU_IN_1, QU_IN_2=QU_IN_2, QU_OUT=QU_OUT, N_CLK=N_CLK, QU_MODE=QU_MODE, OF_MODE=OF_MODE, IF_RST_N=IF_RST_N, PORTS={'i_data_1': 'a_im', 'i_data_2': 'b_re', 'o_data': 'prod_ai_br', **({'i_clk': 'i_clk', **({'i_rst_n': 'i_rst_n'} if IF_RST_N else {})} if N_CLK > 0 else {})})  # type: ignore
        ...
        ModuleMul(QU_IN_1=QU_IN_1, QU_IN_2=QU_IN_2, QU_OUT=QU_OUT, N_CLK=N_CLK, QU_MODE=QU_MODE, OF_MODE=OF_MODE, IF_RST_N=IF_RST_N, PORTS={'i_data_1': 'a_re', 'i_data_2': 'b_re', 'o_data': 'k1', **({'i_clk': 'i_clk', **({'i_rst_n': 'i_rst_n'} if IF_RST_N else {})} if N_CLK > 0 else {})})  # type: ignore

        # k2 = a_im * b_im
        #/ wire [`COMP_DWT_O`-1:0] k2;
        ModuleMul(QU_IN_1=QU_IN_1, QU_IN_2=QU_IN_2, QU_OUT=QU_OUT, N_CLK=N_CLK, QU_MODE=QU_MODE, OF_MODE=OF_MODE, IF_RST_N=IF_RST_N, PORTS={'i_data_1': 'a_im', 'i_data_2': 'b_im', 'o_data': 'k2', **({'i_clk': 'i_clk', **({'i_rst_n': 'i_rst_n'} if IF_RST_N else {})} if N_CLK > 0 else {})})  # type: ignore

        # k3 = s1 * s2
        #/ wire [`COMP_DWT_O`-1:0] k3;
        ModuleMul(QU_IN_1=QU_PRE_1, QU_IN_2=QU_PRE_2, QU_OUT=QU_OUT, N_CLK=N_CLK, QU_MODE=QU_MODE, OF_MODE=OF_MODE, IF_RST_N=IF_RST_N, PORTS={'i_data_1': 's1', 'i_data_2': 's2', 'o_data': 'k3', **({'i_clk': 'i_clk', **({'i_rst_n': 'i_rst_n'} if IF_RST_N else {})} if N_CLK > 0 else {})})  # type: ignore
