...
def build_fi_core_graph(
    *,
    ...
    # The estimator only needs the architectural multiplier latency.  Keep
    # that timing constant local so area estimation does not require the RTL
    # generator module (``v_core_lmmse_interp``) to be installed.
    MUL_LATENCY = 1
...
def analyze_interp_timing(
    *,
    ...
    if freq_interp_method == 'nn':
        ...
        _, fi_info = build_fi_core_graph(
            N_PILOTS_PER_RB=6, LMMSE_P=FI_LMMSE_P,
            RB_PARALLELISM=FI_RB_PARALLELISM, H_DWT=Qu_H_DWT,
            COEFF_DWT=FI_LMMSE_COEFF_DWT,
            REAL_COEFF=FI_LMMSE_REAL_COEFF,
            FI_RE_PARALLELISM=FI_RE_PARALLELISM,
            has_pre_fi_buf=has_pre_fi_buf,
        )
        freq_depth = fi_info['total_depth']
    else:
        freq_depth = 1
...
def analyze_ls_timing(
    *,
    ...
    if freq_interp_method == 'nn':
        ...
        _, fi_info = build_fi_core_graph(
            N_PILOTS_PER_RB=_n_pilots_per_rb, LMMSE_P=LMMSE_P,
            RB_PARALLELISM=RB_PARALLELISM, H_DWT=QU_H_LS.DWT,
            COEFF_DWT=COEFF_DWT_LMMSE,
            REAL_COEFF=REAL_COEFF_LMMSE,
            FI_RE_PARALLELISM=FI_RE_PARALLELISM,
        )
        N_CLK_FREQ = fi_info['total_depth']
    else:
        N_CLK_FREQ = 1
