...
def ModuleCORE_TIME_NN_INTERP(max_occasions: int, Qu_H: QuType, dmrs_typeA_pos: str, is_double_dmrs: bool | Literal["Hybrid"], additional_DMRS_range: List[int], num_symbols_range: List[int]) -> None:
    ...
    **Latency**: 1 clock cycle (registered output).
    ...
    if single_table:
        ...
        ModuleDelay(DWT=TOTAL_W, N_CLK=1, IF_RST_N=False, PORTS={'i_data': 'nn_concat_in', 'o_data': 'nn_concat_out', 'i_clk': 'clk'})  # type: ignore
        for sym in range(N_OUT_SYMS):
            lo = sym * H_DWT
            hi = lo + H_DWT - 1
        ...
        ModuleDelay(DWT=TOTAL_W_MT, N_CLK=1, IF_RST_N=False, PORTS={'i_data': 'nn_mt_concat_in', 'o_data': 'nn_mt_concat_out', 'i_clk': 'clk'})  # type: ignore
        for sym in range(N_OUT_SYMS):
            lo = sym * H_DWT
            hi = lo + H_DWT - 1
