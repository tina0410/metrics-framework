...
def ModuleAdderTree(QU_IN: QuType | list[QuType], QU_OUT: QuType, N_PIPELINES: int | list, QU_MODE: QuMode.TRN | QuMode.RND | list, OF_MODE: OfMode.WRP | OfMode.SAT | list, IF_RST_N: bool, N_INPUTS: int, CONFIG_MODE: Literal['A'] | Literal['B'] | Literal['C']):
    ...
    **Mode C (Reserved):**
    ...
    if not isinstance(CONFIG_MODE, str):
        raise TypeError("CONFIG_MODE must be a string.")
    CONFIG_MODE = CONFIG_MODE.upper()
    ...
    if CONFIG_MODE == "A":
        ...
        if isinstance(N_PIPELINES, bool) or N_PIPELINES < 0:
            raise ValueError("Mode A N_PIPELINES must be a non-negative integer.")
        if not isinstance(IF_RST_N, bool):
            raise TypeError("IF_RST_N must be bool.")
        ...
        if not (
            len(QU_IN) == len(N_PIPELINES) == len(QU_MODE) == len(OF_MODE) == n_layers
        ):
            raise ValueError("Mode B requires exactly one entry per tree layer.")
