...
#     - Delay (1.0.0)
# Release Notes:
#     - 1.0.0: Fixed round-mode behavior and nonnegative saturation handling.
###################################################################################################
import pytv
...
def ModuleFxMatch(QU_IN: QuType, QU_OUT: QuType, QU_MODE: QuMode.TRN | QuMode.RND, OF_MODE: OfMode.WRP | OfMode.SAT, N_CLK: int, IF_RST_N: bool):
    ...
    # Convert the internal signed-equivalent format to the requested output.
    ...
    if QU_OUT.IF_SIGNED == False:
        if OF_MODE_E == OfMode.WRP.TCPL:
            ...
            # Saturating nonnegative outputs clamp every negative result to 0.
            #/ // Clamp a negative result before dropping the auxiliary sign bit
            #/ assign o_data_trunc = o_data_eq[`EQ_QU_OUT.DWT`-1] ? `QU_OUT.DWT`'b0 : o_data_eq[`EQ_QU_OUT.DWT`-2:0];
            pass
    else:
