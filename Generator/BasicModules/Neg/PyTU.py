...
def same_qu_type(left, right):
    """Return whether two ``QuType`` objects describe the same fixed-point type."""
    fields = ("DWT", "FRAC", "IF_SIGNED")
    return all(hasattr(left, field) and hasattr(right, field) for field in fields) and all(
        getattr(left, field) == getattr(right, field) for field in fields
    )


__all__ = ["QuMode", "OfMode", "QuType", "same_qu_type"]
