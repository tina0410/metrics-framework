...
public:
    ...
    inline auto fill()
    {
        ...
        // Qu_s reserves one internal sign bit even for unsigned formats.  The
        // random ArbiInt fill also randomizes that hidden bit, which makes an
        // unsigned value negative during multiplication while toString()
        // silently omits it.  Clear all storage above the visible unsigned
        // width so the C++ reference has the same semantics as the RTL ports.
        if constexpr (!isS)
        {
            constexpr size_t visibleBits = intB + fracB;
            if constexpr (decltype(data)::num_bits <= 64)
            {
                data.data = static_cast<typename decltype(data)::data_t>(
                    static_cast<uint64_t>(data.data) & ((uint64_t(1) << visibleBits) - 1));
            }
            else
            {
                constexpr size_t completeWords = visibleBits / 64;
                constexpr size_t remainingBits = visibleBits % 64;
                if constexpr (remainingBits != 0)
                {
                    data.data[completeWords] &= (uint64_t(1) << remainingBits) - 1;
                }
                for (size_t index = completeWords + (remainingBits != 0); index < data.num_words; ++index)
                {
                    data.data[index] = 0;
                }
            }
