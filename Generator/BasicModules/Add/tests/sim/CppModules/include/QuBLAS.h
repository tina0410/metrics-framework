...
public:
    ...
    inline auto fill()
    {
        ...
        if constexpr (!isS)
        {
            // Qu_s keeps one internal guard/sign bit even for unsigned data.
            // Random input generation must not set that bit.
            this->data = this->data & decltype(this->data)::maximum();
        }
