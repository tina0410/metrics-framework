...
\version{0.1.0}
\revision{      
                0.1.0   & Bolin Li   & 2024.11.25    & Delete all redundant contents and descriptions in \textsc{Sub} Module\\   
}
\releasenotes{
...
\begin{document}
    ...
    \chapter{Preliminaries}
        ...
        This module will realize a general purpose \textbf{fixed-point subtraction} operation. It will take two input specified fixed-point value and calculate their subtraction in specified fixed-point format.
        \section{Notations}
            \begin{itemize}
    ...
    \chapter{Functionality}
        ...
        \section{External interface}
            ...
            \subsection{Main Module Interface}
                \begin{longtable}{lll}
                    ...
                    \texttt{i\_ctrl\_en}     & $1$                 & Enable signal for the Sub module\\
                    \bottomrule
                \end{longtable}
        ...
        \begin{algorithm}[H]
            ...
            \BlankLine
            \tcp{Minus data\_2 for signed/unsigned situations}
            \If{\texttt{QU\_IN\_2.IF\_SIGNED} == True}{
                \tcp{Use Two's Complement}
                \texttt{data\_2\_minus = \~\{data\_2[QU\_IN\_2.DWT-1], data\_2\} + 1'b1;}
            }\Else{
                \texttt{data\_2\_minus = \~\{1'b0, data\_2\} + 1'b1; }
            }
            
            \tcp{Calculate Quantization Set for adding from input data}
            \texttt{DWT\_FIX = max(IntBits(QU\_IN\_1), IntBits(QU\_IN\_2)+1) + max(FracBits(QU\_IN\_1), FracBits(QU\_IN\_2)) + 1;} \tcp{Minus operation add a bit to data\_2}
            \BlankLine
            \texttt{FRAC\_FIX = max(QU\_IN\_1.FRAC, QU\_IN\_2.FRAC)};
            \BlankLine
            ...
            \BlankLine
            \texttt{data\_1\_fixed = data\_1.FxMatch(QU\_IN = QU\_IN\_1, QU\_OUT = QU\_IN\_FIX)};
            \BlankLine
            \texttt{data\_2\_fixed\_minus = data\_2\_minus.FxMatch(QU\_IN=QU\_IN\_2, QU\_OUT=QU\_IN\_FIX)};
            \BlankLine
            \tcp{Adder}
            \texttt{result\_unfixed = data\_1\_fixed + data\_2\_fixed\_minus;}
            \BlankLine
    ...
    \chapter{Implementation}
        \section{Overall architecture}
                The \textsc{Sub} Module relies on the \textsc{FxMatch} module to perform the fixed-point subtraction operation. The \textsc{FxMatch} module takes two input values with configurable precisions, then output their sum in a configurable precision fixed-point format. The \textsc{Sub} Module then applies the specified quantization and overflow modes to the output value.
