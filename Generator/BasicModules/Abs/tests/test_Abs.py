...
# Version: 1.0.0
# Date: 2026.9.1
# Description: Pytest verification for Abs
...
import PyTB
import PyTU
from BehavModel_Abs import (
    ModuleCppConfigAbs as ModuleCppConfig,
    ModuleCppRunAbs as ModuleCppRun,
)
from tb_Abs import ModuleTbAbs
...
class Testcase:
    __test__ = False
    DUT_NAME = "Abs"
    CPP_CONFIG_FILE_NAME_PREFIX = "CppConfigAbs"
    CPP_RUN_FILE_NAME_PREFIX = "CppRunAbs"
    CPP_CONFIG_FILE_NAME_DEST = "config.h"
    CPP_RUN_FILE_NAME_DEST = "Abs.cpp"
...
def generate_testcases():
    return [
        ...
        Testcase(
            "nonnegative_to_signed",
            PyTU.QuType(8, 2, False),
            PyTU.QuType(9, 2, True),
            PyTU.QuMode.RND.INF,
            PyTU.OfMode.SAT.TCPL,
            True,
            1,
        ...
        Testcase(
            "convergent_narrow_zero",
            PyTU.QuType(9, 5, True),
            PyTU.QuType(4, 2, True),
            PyTU.QuMode.RND.CONV,
            PyTU.OfMode.SAT.ZERO,
            True,
            2,
...
def test_abs(case: Testcase):
    ...
    generated_config = (
        list(moduleloader.getParams(case.CPP_CONFIG_FILE_NAME_PREFIX)[-1].keys())[-1]
        + ".h"
    )
    generated_runner = (
        list(moduleloader.getParams(case.CPP_RUN_FILE_NAME_PREFIX)[-1].keys())[-1]
        + ".cpp"
    )
    os.replace(
        cpp_include_dir / generated_config,
        cpp_include_dir / case.CPP_CONFIG_FILE_NAME_DEST,
    )
    os.replace(
        cpp_dir / generated_runner,
        cpp_dir / case.CPP_RUN_FILE_NAME_DEST,
    )
    moduleloader.reset()
    ...
    input_file = input_dir / "abs_i_data.txt"
    expected_file = comparison_dir / "abs_o_data.txt"
    actual_file = output_dir / "abs_o_data.txt"
    inputs = input_file.read_text(encoding="utf-8").splitlines()
    expected = expected_file.read_text(encoding="utf-8").splitlines()
    actual = actual_file.read_text(encoding="utf-8").splitlines()
...
def compare_streams(
    ...
    for line_number, (input_bits, expected_bits, actual_bits) in enumerate(
        ...
        if expected_bits == actual_bits and None not in (
            input_bits,
            expected_bits,
            actual_bits,
        ):
