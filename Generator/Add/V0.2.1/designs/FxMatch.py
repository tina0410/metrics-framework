...
def ModuleFxMatch(QU_IN, QU_OUT, QU_MODE = QuMode.TRN.TCPL, OF_MODE = OfMode.WRP.TCPL, N_CLK = 0, IF_RST_N = False):
    ...
    if EQ_QU_OUT.LSB() > EQ_QU_IN.MSB():
        ...
        #/ // Quantization Stage
        if OF_MODE == OfMode.SAT.SMGN and EQ_QU_OUT.DWT == 1:
            #/ assign o_data_eq = 1'b0;
            pass
        elif EQ_QU_OUT.LSB() - EQ_QU_IN.MSB() > 1:
            ...
            match QU_MODE:
                case QuMode.TRN.TCPL | QuMode.TRN.SMGN:
                    ...
                    pass
                case QuMode.RND.POS_INF | QuMode.RND.ZERO | QuMode.RND.CONV | QuMode.RND.NEG_INF | QuMode.RND.INF:
            ...
            match QU_MODE:
                case QuMode.TRN.TCPL | QuMode.TRN.SMGN:
                    #/ assign o_data_eq = i_data_eq[`EQ_QU_IN.DWT`-1] ? {`EQ_QU_OUT.DWT`{1'b1}} : {`EQ_QU_OUT.DWT`{1'b0}};
                    pass
                case QuMode.RND.POS_INF | QuMode.RND.ZERO | QuMode.RND.CONV:
                    #/ assign o_data_eq = {`EQ_QU_OUT.DWT`{1'b0}};
                    pass
        ...
        if EQ_QU_OUT.LSB() > EQ_QU_IN.LSB():
            ...
            match QU_MODE:
                ...
                case QuMode.TRN.SMGN:
                    ...
                    if QU_QUAN.DWT==1:
                        #/ assign op_quan = i_data_eq[`EQ_QU_IN.DWT`-1] ? 1'b1 : op_quan_abs;
                        pass
                    else:
