...
\modulename{Add}
\author{Bolin Li and Yifang Dai}
\date{2025/3/26}
\version{0.2.1}
\revision{      
                0.1.1   & Bolin Li   & 2024/11/03    & Initial Update \\
                0.1.2   & Bolin Li   & 2024/11/04    & Complete the interface part of the document, and major revise of the functionality part. \\
                0.1.3   & Bolin Li   & 2024/11/06    & Redefine enable signal as a switch on input data signals, Small fixes of doc\\
                0.2.1   & Yifang Dai   & 2025/03/25    & Overall adjustment and optimization (deleted enable signal)\\
...
\begin{document}
    ...
    \chapter{Preliminaries}
        ...
        This module will realize a general purpose \textbf{fixed-point addition} operation. It takes two fixed-point inputs and computes their sum in the specified fixed-point format.
        \section{Notations}
            All definitions, acronyms, and abbreviations used in the document should be listed here.
            \begin{itemize}
                \item LSB: Least significant bit.
                \item MSB: Most significant bit.
                \item TCPL: Two's complement format
        ...
        \section{Dependencies}
            ...
            \subsection{Software dependencies}
                This part lists all the software dependencies of the module, including the software name and version.
                \begin{longtable}{ll}
                    \toprule
    ...
    \chapter{Functionality}
        \section{Overview}
            The \textsc{Add} Module receives two input values with configurable precisions, then output their sum in a configurable precision fixed-point format. Now the module supports quantization modes and overflow modes listed in section ~\ref{ch:func:sec:param:subsec:QuMode}~ and ~\ref{ch:func:sec:param:subsec:OfMode}~, respectively.  The module also supports optional \textbf{negedge reset}, which can be configured by \texttt{IF\_RST\_N} parameter, respectively.
        ...
        \begin{algorithm}[H]
            ...
            \BlankLine
                ...
                \BlankLine
            \tcp{Adder}
            \texttt{result\_unfixed = data\_1 + data\_2;}
            \BlankLine
            \tcp{Quantize and handle overflow to get the specified-format final result}
         
            \texttt{o\_result = result\_unfixed.FxMatch(o\_result.DWT, o\_result.FRAC, o\_result.SIGNED)};
    ...
    \chapter{Implementation}
        \section{Overall architecture}
                The \textsc{Add} Module relies on the \textsc{FxMatch} module to perform the fixed-point addition operation. The \textsc{FxMatch} module takes two input values with configurable precisions, then output their sum in a configurable precision fixed-point format. The \textsc{Add} Module then applies the specified quantization and overflow modes to the output value.
