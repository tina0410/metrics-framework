...
# from utils import run_iverilog_flow, merge_files
...
@ convert
def Moduletest(width, N, M, archi, algo):
    #/ `timescale 1ns / 1ps
    #/ module test;
    #/ reg clk;
    #/ reg rst;
    ...
    clk_period = 20
    clk_period_half = int(clk_period/2)
    #/ initial
    #/ begin
    #/     clk = 1'b0;
    #/     rst = 1'b0;
    ...
    if N == M:
        ...
        #/ initial
        #/ begin
        #/     file=$fopen("D:\\\\ChannelCoding\\\\AutoGen\\\\BPPolarDecoder\\\\FinalTests\\\\tb\\\\test_files\\\\R.txt","r");
        ...
        #/     $fclose(file);
        if archi == 'TypeII':
            #/     file=$fopen("D:\\\\ChannelCoding\\\\AutoGen\\\\BPPolarDecoder\\\\FinalTests\\\\tb\\\\test_files\\\\L.txt","r");
            #/     for(i=0;i<(`N*width`);i=i+1) begin
            #/         ret = $fscanf(file,"%b",temp);
    ...
    # In case when N>M, repetitive input dump is required
        ...
        num_iter = 100
        input_file_path_R = "D:\\ChannelCoding\\AutoGen\\BPPolarDecoder\\FinalTests\\tb\\test_files\\R.txt"
        input_file_path_L = "D:\\ChannelCoding\\AutoGen\\BPPolarDecoder\\FinalTests\\tb\\test_files\\L.txt"
        R_= []
        L = []
    ...
    #/ always @(posedge clk) 
    #/ begin
    #/     file=$fopen("D:\\\\ChannelCoding\\\\AutoGen\\\\BPPolarDecoder\\\\FinalTests\\\\tb\\\\test_files\\\\Decoding_bit_pytv_M4_TypeII_Fin1.txt","w");
    #/     if(out_valid==1'b1)
    #/     begin
    #/         #10
    #/         for(i=0;i<`N`;i=i+1)
