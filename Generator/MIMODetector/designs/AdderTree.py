...
def ModuleAdderTree(QU_IN:QuType, QU_OUT:QuType, N_PIPELINES = 10, QU_MODE = QuMode.TRN.TCPL, OF_MODE = OfMode.WRP.TCPL, IF_RST_N = False, N_INPUTS=2, CONFIG_MODE = "A"):
    ...
    if CONFIG_MODE == "A":
        ...
        N_PIPELINES = [base_pipelines] * n_layers

        # 先保证首尾各 +1（如果 extra_pipelines >= 2）
        if extra_pipelines >= 2:
            N_PIPELINES[0] += 1  # 首层 +1
            N_PIPELINES[-1] += 1  # 尾层 +1
            extra_pipelines -= 2
        elif extra_pipelines == 1:
            N_PIPELINES[0] += 1  # 如果只有 1 个，优先给首层
            extra_pipelines -= 1

        # 剩余的 extra_pipelines 均匀分配到中间层
        if extra_pipelines > 0:
            step = (n_layers - 2) / extra_pipelines/2  # 中间层步长（去掉首尾）
            for i in range(1,extra_pipelines+1):
                pos = 1 + int(i * step)  # 从第 2 层（i=1）开始
                if pos >= n_layers - 1:  # 避免越界（不能覆盖尾层）
                    pos = n_layers - 2   # 放在倒数第二层
                N_PIPELINES[pos] += 1
        # base_pipelines = N_PIPELINES // n_layers
        # extra_pipelines = N_PIPELINES % n_layers
        # N_PIPELINES = [base_pipelines if i < n_layers - extra_pipelines else base_pipelines + 1 for i in range(n_layers)]
        print("N_PIPELINES",N_PIPELINES)
