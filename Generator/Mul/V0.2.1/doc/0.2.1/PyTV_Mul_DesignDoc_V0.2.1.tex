...
\revision{      
                ...
                0.1.1   & Bolin Li   & 2024.11.29    & Completed the doc, major revise\\
                0.2.1   & Yifang Dai   & 2025.3.28    & Overall optimization\\
}
\releasenotes{
...
\begin{document}
    ...
    \chapter{Functionality}
        ...
        \section{Configurable parameters}
            \begin{longtable}{llll}
                ...
                \texttt{QU\_IN\_1}     & $[\mathbb{N}^{+}, \mathbb{Z}, \mathbb{B}]$  & \texttt{[9,3,True]}                      &  Data 1 quantization set \\
                \texttt{QU\_IN\_2}    & $[\mathbb{N}^{+}, \mathbb{Z}, \mathbb{B}]$  & \texttt{[9,3,True]}                      & Data 2 quantization set \\
        ...
        \section{External interface}
            ...
            \subsection{Main Module Interface}
                \begin{longtable}{lll}
                    ...
                    \textbf{Signal}      & \textbf{Width}    & \textbf{Description} \\
                    \midrule
                    \texttt{i\_data\_1}      & \texttt{QU\_IN\_1.DWT}         & Input signal for data 1 \\
                    \texttt{i\_data\_2}      & \texttt{QU\_IN\_2.DWT}        & Input signal for data 2 \\
                    \texttt{o\_data}    & \texttt{QU\_OUT.DWT}         & Output signal for result \\
                    \bottomrule
                \end{longtable}
        ...
        \begin{algorithm}[H]
            ...
            \BlankLine
            \tcp{Multiply}
            \texttt{$\operatorname{abs}$(o\_data\_unfixed)=$ \operatorname{abs}$(i\_data\_1) $\times$ $ \operatorname{abs}$(i\_data\_2)}
            \BlankLine
            \If{\texttt{QU\_IN\_1.IF\_SIGNED} \&\& \texttt{QU\_IN\_2.IF\_SIGNED}}{
               $ \operatorname{sign}$\texttt{(o\_data\_unfixed) = $ \operatorname{sign}$(i\_data\_1) $\oplus$ $ \operatorname{sign}$(i\_data\_2)}
            \BlankLine
            }\ElseIf{\texttt{QU\_IN\_1.IF\_SIGNED}}{
                \texttt{$\operatorname{sign}$ (o\_data\_unfixed) = $ \operatorname{sign}$(i\_data\_1)}
            \BlankLine
            }\ElseIf{\texttt{QU\_IN\_2.IF\_SIGNED}}{
                \texttt{$\operatorname{sign}$(o\_data\_unfixed) = $\operatorname{sign}$(i\_data\_2)}
            \BlankLine
            }\Else{
            ...
            \texttt{o\_data = o\_data\_unfixed.FxMatch(\texttt{QU\_OUT\_DWT, QU\_OUT\_FRAC, QU\_OUT\_SIGNED})};
    ...
    \chapter{Implementation}
        \section{Overall architecture}
                The \textsc{Mul} Module relies on the \textsc{FxMatch} module to perform the fixed-point multiplication operation. The \textsc{FxMatch} module takes two input values with configurable precisions, then output their multiplied value in a configurable precision fixed-point format. The \textsc{Mul} Module then applies the specified quantization and overflow modes to the output value.
